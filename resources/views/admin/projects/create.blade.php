...
<div style="max-width:900px">
  ...
  <form method="POST"
    ...
    <div class="card" style="margin-bottom:1.25rem">
      <div class="card__header"><h3>Proje Bilgileri</h3></div>
      <div class="card__body" style="padding:1.5rem">
        <div class="form-row">
          <div class="form-group">
            ...
            <select name="service_id" class="form-control">
              ...
              <option value="{{ $s->id }}" {{ (old('service_id', $project->service_id ?? null) == $s->id) ? 'selected' : '' }}>{{ $s->title_tr }}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="number" name="year" class="form-control" min="2000" max="2099" value="{{ old('year', $project->year ?? date('Y')) }}">
          </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            @if(isset($project) && $project->cover_image)
              <img src="{{ asset('storage/' . ltrim($project->cover_image, 'public/')) }}" style="max-width:200px;margin-top:.5rem;border-radius:4px" onerror="this.style.display='none'">
            @endif
          </div>
          <div class="form-group">
            ...
            @if(isset($project) && $project->gallery->count())
              <div style="display:flex;gap:.5rem;margin-top:.5rem;flex-wrap:wrap">
                @foreach($project->gallery as $img)
                  <img src="{{ asset('storage/' . ltrim($img->image_path, 'public/')) }}" style="max-height:50px;border-radius:4px" onerror="this.style.display='none'">
                @endforeach
              </div>
            @endif
          </div>
      ...
      <div class="card__body" style="padding:1.5rem">
        <div id="tab-tr">
          <div class="form-group">
            ...
            <input type="text" name="title_tr" class="form-control" required value="{{ old('title_tr', $project->title_tr ?? '') }}">
          </div>
          <div class="form-row">
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <input type="text" name="size_tr" class="form-control" placeholder="Örn: 80.000 m² veya 7.500 kişilik" value="{{ old('size_tr', $project->size_tr ?? '') }}">
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">İşveren / Müşteri (TR)</label>
              <input type="text" name="client_tr" class="form-control" value="{{ old('client_tr', $project->client_tr ?? '') }}">
            </div>
            <div class="form-group">
              <label class="form-label">Süre / Tarih (TR)</label>
              <input type="text" name="duration_tr" class="form-control" value="{{ old('duration_tr', $project->duration_tr ?? '') }}">
            </div>
          </div>
          <div class="form-group">
            <label class="form-label">Kapsam (TR)</label>
            <textarea name="scope_tr" class="form-control" rows="2">{{ old('scope_tr', $project->scope_tr ?? '') }}</textarea>
          </div>
          <div class="form-group">
            <label class="form-label">Açıklama (TR)</label>
            <textarea name="description_tr" class="form-control" rows="8">{{ old('description_tr', $project->description_tr ?? '') }}</textarea>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">Meta Başlık (TR)</label>
              <input type="text" name="meta_title_tr" class="form-control" value="{{ old('meta_title_tr', $project->meta_title_tr ?? '') }}">
            </div>
            <div class="form-group">
              <label class="form-label">Meta Açıklama (TR)</label>
              <textarea name="meta_desc_tr" class="form-control" rows="2">{{ old('meta_desc_tr', $project->meta_desc_tr ?? '') }}</textarea>
            </div>
          </div>
        </div>
        <div id="tab-en" style="display:none">
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input type="text" name="size_en" class="form-control" value="{{ old('size_en', $project->size_en ?? '') }}">
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">Client (EN)</label>
              <input type="text" name="client_en" class="form-control" value="{{ old('client_en', $project->client_en ?? '') }}">
            </div>
            <div class="form-group">
              <label class="form-label">Duration/Date (EN)</label>
              <input type="text" name="duration_en" class="form-control" value="{{ old('duration_en', $project->duration_en ?? '') }}">
            </div>
          </div>
          <div class="form-group">
            <label class="form-label">Scope (EN)</label>
            <textarea name="scope_en" class="form-control" rows="2">{{ old('scope_en', $project->scope_en ?? '') }}</textarea>
          </div>
          <div class="form-group">
            <label class="form-label">Description (EN)</label>
            <textarea name="description_en" class="form-control" rows="8">{{ old('description_en', $project->description_en ?? '') }}</textarea>
          </div>
          <div class="form-row">
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <textarea name="meta_desc_en" class="form-control" rows="2">{{ old('meta_desc_en', $project->meta_desc_en ?? '') }}</textarea>
            </div>
          </div>
        </div>
      </div>
